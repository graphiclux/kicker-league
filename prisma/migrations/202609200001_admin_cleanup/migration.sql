-- Archived leagues are retained for audit/history but hidden from normal users.
ALTER TABLE "League" DROP CONSTRAINT IF EXISTS "league_status";
ALTER TABLE "League" ADD CONSTRAINT "league_status" CHECK (status IN ('LOBBY','DRAFTING','PAUSED','COMPLETE','ARCHIVED'));

-- A team name is presentation metadata and may be corrected after a draft;
-- ownership, league membership, draft order, and rankings stay immutable.
